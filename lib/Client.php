...
use Journify\Consumer\Consumer;
use Journify\Consumer\File;
use Journify\Consumer\ForkCurl;
use Journify\Consumer\LibCurl;
use Journify\Consumer\Socket;
...
class Client
{
    ...
    public function __construct(string $secret, array $options = [])
    {
        ...
        if (!array_key_exists($consumer_type, $consumers) && class_exists($consumer_type)) {
            if (!is_subclass_of($consumer_type, Consumer::class)) {
                throw new JournifyException('Consumers must extend the Journify/Consumer/Consumer abstract class');
            }
    ...
    /**
     * Add the journify.io context to the request
    ...
    private function getDefaultContext(): array
    {
        ...
        global $JOURNIFY_VERSION;

        return [
            'library' => [
                'name'     => 'journify-php',
                'version'  => $JOURNIFY_VERSION,
