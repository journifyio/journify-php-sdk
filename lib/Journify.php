...
class Journify
{
    ...
    {
        self::assert($secret, 'Journify::init() requires secret');
     ...
     * @param mixed $value
     * @param string $msg
     * @throws JournifyException
     */
    private static function assert($value, string $msg): void
    {
        if (empty($value)) {
            throw new JournifyException($msg);
        }
    }
    ...
    public static function track(array $message): bool
    {
        ...
        self::assert($event, 'Journify::track() expects an event');
    ...
    private static function checkClient(): void
    {
        ...
        throw new JournifyException('Journify::init() must be called before any other tracking method.');
    }
    ...
    public static function validate(array $message, string $type): void
    {
        ...
        self::assert($userId || $anonId, "Journify::$type() requires userId or anonymousId");
    }
    ...
    public static function group(array $message): bool
    {
        ...
        self::assert($groupId, 'Journify::group() expects a groupId');
    ...
    public static function alias(array $message): bool
    {
        ...
        self::assert($userId && $previousId, 'Journify::alias() requires both userId and previousId');
